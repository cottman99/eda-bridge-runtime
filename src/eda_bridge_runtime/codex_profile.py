...
def render_profile(
    ...
    *,
    ...
    if any(char in runtime_command for char in ('"', "\r", "\n")):
        raise ValueError("runtime command cannot contain quotes or newlines")
    hook_runtime_command = (
        f'"{runtime_command}"'
        if any(char.isspace() for char in runtime_command)
        else runtime_command
    )
    pre_hook_command = f"{hook_runtime_command} hook codex-pre-tool-use"
    post_hook_command = f"{hook_runtime_command} hook codex-post-tool-use"
    lines = [
