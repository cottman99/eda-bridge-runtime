...
def render_profile(
    ...
    *,
    ...
    if runtime_command is None:
        ...
        module_args = ["-m", "eda_bridge_runtime.cli"]
    if not runtime_command or any(char in runtime_command for char in ('"', "\r", "\n")):
        raise ValueError("runtime command must be non-empty and cannot contain quotes or newlines")
    hook_runtime_command = (
        f'"{runtime_command}"'
        if any(char.isspace() for char in runtime_command)
        else runtime_command
    )
    module_prefix = " ".join(module_args)
    hook_prefix = (
        f"{hook_runtime_command} {module_prefix}" if module_prefix else hook_runtime_command
    )
    pre_hook_command = f"{hook_prefix} hook codex-pre-tool-use"
    post_hook_command = f"{hook_prefix} hook codex-post-tool-use"
    lines = [
        ...
        f"args = {json.dumps([*module_args, 'mcp', 'serve'])}",
        "required = true",
        ...
        'type = "command"',
        f"command = {json.dumps(pre_hook_command)}",
        "timeout = 3",
        "",
        "[[hooks.PostToolUse]]",
        'matcher = "^mcp__eda_bridge_runtime__.*$"',
        ...
        'type = "command"',
        f"command = {json.dumps(post_hook_command)}",
        "timeout = 3",
        "",
    ]
...
def main() -> int:
    ...
    parser.add_argument(
        "--runtime-command",
        help=(
            "Exact Runtime executable override. By default the generated profile launches "
            "the installing Python with -m eda_bridge_runtime.cli."
        ),
    )
    parser.add_argument(
        "--approve-mutations",
